...
This default static path is for no-build apps. If the group has chosen a framework (e.g. Next.js or Vite), scaffold that instead of copying `starter.html`.

Copy `starter.html` to `index.html` in the project root if `index.html` doesn't already exist. This gives the group a working styled page using the Base Design System to build on immediately. Tell the group to open `index.html` in their browser now so they can see changes as they build.
