...
**Shape (working plan, may change):** about 90 minutes total - a 10-minute opening from the organisers, a 60-minute build session run in parallel by all groups (4-5 participants and one facilitator per table), then a 20-minute present-back from three to five groups.
...
| Organiser | Confirm headcount and build the group list - 4-5 participants per group, one facilitator each. Names on tables. |
| Organiser | Brief whoever is opening on the 10-minute opening (see "The opening" below). Keep it to 10. |
...
| Organiser | Print: [DAY_PLAN.md](DAY_PLAN.md) for every participant, [FACILITATOR_GUIDE.md](FACILITATOR_GUIDE.md) and this run sheet for every facilitator. Optional: [SCENE_SETTER.md](SCENE_SETTER.md) sent round in advance. |
...
|---|---|---|---|
| 0:00 | Opening (10 min) | The organisers | See "The opening". Hard stop at 10 minutes - the build session needs its full hour. |
...
| 1:30 | Close | The organisers | One or two lines: what happens to the ideas now (route it through your organisation's innovation or IT team - see [TAKE_BACK_TO_YOUR_TEAM.md](TAKE_BACK_TO_YOUR_TEAM.md)), thanks, done. Fold this into the present-back slot if time is tight. |

---

## The opening (the organisers, 10 minutes)
...
What it lands, in order: why you're here (not to become developers - the art of the possible) - what vibe coding is - the shift (idea to working version in an afternoon, not months) - why the people closest to the friction are the right ones to do this - more time on the real work, less on the operational drag around it - what makes a good idea (real, specific, contained) and the themes - how the next 80 minutes work - close ("what's the most annoying thing I do repeatedly that a computer should be doing?").
...
The [SHIPPING_CHECKLIST.md](SHIPPING_CHECKLIST.md) conversation doesn't fit in 60 minutes - point groups at [TAKE_BACK_TO_YOUR_TEAM.md](TAKE_BACK_TO_YOUR_TEAM.md) and [SHIPPING.md](SHIPPING.md) to read afterwards instead.
...
If you have a spare two minutes at the end, the organisers close with the "what happens to these ideas now" line. If not, the organiser sends it round by email afterwards.

---

## What "done" looks like

By 1:10 every group should have an `index.html` that runs and does something, and a `presentation.html` that opens - whether or not they present. By 1:30 every participant should leave understanding that they can spot a real problem, describe it well enough for an AI to build something, get to a prototype in under an hour, and that taking it further means a different set of conversations - IT, security, data, a sponsor. The code is the proof of concept. That mental model shift is the point.
